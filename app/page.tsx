...
const NAV = [
  ...
  { id: "tenders" as PageId, icon: "◈", label: "Tenders", badge: 8 },
...
export default function Page() {
  ...
  return (
    <div className="flex h-screen overflow-hidden text-white" style={{ background: "linear-gradient(135deg,#020c18 0%,#041220 50%,#020e1a 100%)" }}>
      {!isUniverse && (
        <>
          <div className="fixed inset-0 pointer-events-none" style={{ backgroundImage: "linear-gradient(rgba(0,255,135,0.022) 1px,transparent 1px),linear-gradient(90deg,rgba(0,255,135,0.022) 1px,transparent 1px)", backgroundSize: "48px 48px" }} />
          <div className="fixed top-0 left-1/3 w-96 h-96 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle,rgba(0,255,135,0.05) 0%,transparent 70%)" }} />
        </>
      )}

      <aside className="relative z-20 flex flex-col w-60 shrink-0 border-r border-white/5" style={{ background: "rgba(2,10,20,0.92)", backdropFilter: "blur(20px)" }}>
        <div className="p-5 border-b border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg,#00ff87,#00b360)" }}>
        ...
        <nav className="flex-1 p-3 space-y-0.5">
          {NAV.map((n) => (
            ...
            >
              ...
              {n.badge && (
                <span className={cn("text-xs px-1.5 py-0.5 rounded-md font-mono", page === n.id ? "bg-emerald-500/30 text-emerald-300" : "bg-white/[0.08] text-slate-500")}>
                  {n.badge}
                </span>
              )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative z-10">
        {!isUniverse && (
          <header className="flex items-center justify-between px-6 py-3.5 border-b border-white/5 shrink-0" style={{ background: "rgba(2,10,20,0.7)", backdropFilter: "blur(20px)" }}>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-white/[0.08]">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center text-black font-bold text-xs" style={{ background: "linear-gradient(135deg,#00ff87,#60efff)" }}>
                  R
                </div>
